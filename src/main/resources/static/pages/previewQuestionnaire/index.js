...
onload =  async () => {
    ...
    $('.btn-div').append(`
        <button type="button" class="btn btn-primary" onclick="submit('${questionnaire.surveyName}','${question}')">提 交</button>
    `);
...
const addAnswerDetail =(answerMessageId,option) =>{

    let selectedValue;
    const textareaElement = document.getElementById(option[0].id);
    let textareaValue = textareaElement.value;

        for (let i = 0; i < option.length; i++) {

            // if (document.getElementById(option[i].id) != null && typeof document.getElementById(option[i].id) != 'undefined') {
                if (document.getElementById(option[i].id).checked ) {
                    let params = {
                        answerMessageId: answerMessageId,
                        optionId: option[i].id,
                        text: option[i].text
                    }
                    selectedValue = option[i].chooseTerm;
                    $.ajax({
                ...
                else if(textareaValue != null && textareaValue!='on'){
                    let params = {
                        answerMessageId: answerMessageId,
                        optionId: option[i].id,
                        text: textareaValue
...
const submit = async (questionnaireName) =>{
    ...
    for(let i=0;i<question.length;i++) {
        let option = await queryOption(question[i].id);
        addAnswerDetail(answerMessageId, option);
    }
